...
export default function RiwayatSurat() {
  ...
  useEffect(() => {
    fetch("/api/admin/permohonan?riwayat=true")
      .then((res) => res.json())
      .then((data) => {
        setList(data);
        setFiltered(data);
  ...
  useEffect(() => {
    ...
    const hasil = list.filter(
      (item) =>
        item.nama_pemohon?.toLowerCase().includes(keyword) ||
    ...
    <div className="flex h-screen bg-gray-50">
      <aside className="w-64 bg-white border-r p-6">
        ...
        <nav className="space-y-2">
          ...
          <div>
            <button className="w-full text-left px-4 py-2 rounded bg-green-600 text-white">
              Permohonan
            </button>
      ...
      <main className="flex-1 p-6 overflow-auto">
        ...
        {/* Input pencarian tambahan di atas tabel */}
        <div className="mb-4">
          <input
            type="text"
